...
try {
    $stmt = $pdo->prepare(
        "SELECT p.*, u.nome_completo, u.avatar_url,
...
<body class="text-dark-text">
                                ...
                                <div class="flex items-start space-x-4">
                                    ...
                                    <div class="flex-grow">
                                        ...
                                        <div class="mt-4 pt-3 border-t border-gray-200 flex items-center space-x-6 text-sm text-light-text">
                                            <button class="curtir-post-button hover:text-primary-blue flex items-center group" data-post-id="<?= $post['id_post']; ?>" data-liked="<?= $post['user_liked']; ?>">
                                                <i class="far fa-thumbs-up mr-1.5 group-hover:text-primary-blue transition-colors <?php if($post['user_liked']) echo 'text-primary-blue'; ?>"></i>
                                                <span class="likes-count ml-0.5"><?= (int)$post['total_curtidas']; ?></span> Curtidas
                                            </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.curtir-post-button').forEach(btn => {
                btn.addEventListener('click', function () {
                    const postId = this.dataset.postId;
                    const params = new URLSearchParams({ id_post: postId });
                    fetch('comunidade_curtir.php', {
                        method: 'POST',
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (icon) icon.classList.toggle('text-primary-blue', data.liked);
                            if (count) count.textContent = data.total;
                        }
                    })
                    .catch(err => console.error('erro curtir', err));
                });
            });
        });
    </script>
